...
function TableHead({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      ...
      className={cn(
        'h-12 px-4 text-left rtl:text-right align-middle font-normal text-mutedForeground [&:has([role=checkbox])]:pe-0',
        className,
      )}
      {...props}
    />
...
function TableCaption({ className, ...props }: React.HTMLAttributes<HTMLTableCaptionElement>) {
  return (
    <caption data-slot="table-caption" className={cn('mt-4 text-sm text-mutedForeground', className)} {...props} />
  );
}
