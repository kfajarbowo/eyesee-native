...
export default function BodyWormDetail() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [camera, setCamera] = useState<BodyWormDetail | null>(null);
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        ...
        console.error('Error fetching Body-Worm detail:', err);
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center h-screen gap-4">
          ...
          <button
            onClick={() => navigate('/body-worm')}
            className="px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition"
          >
            Back to Body-Worm Cameras
          </button>
        </div>
      </DashboardLayout>
    );
