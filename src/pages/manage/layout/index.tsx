...
export default function LayoutListPage() {
  const { user } = useAuth();
  const { isLoading, data, refetch } = useAllLayout();
  const [searchQuery, setSearchQuery] = useState("");
  const [pagination, setPagination] = useState({ pageIndex: 0, pageSize: 5 });

  const filteredData = useMemo(() => {
    const allData = (data?.data as any[]) || [];
  ...
  return (
    <ManageLayoutWrapper>
      <Toolbar>
        <ToolbarHeading />
        <ToolbarActions>
          {hasPermission(user, "layout.create") && (
            <Link to="/manage/layout/create" className="btn">Create</Link>
          )}
        </ToolbarActions>
      </Toolbar>
      <Container>
        <div className="bg-card rounded-xl border border-border">
          <div className="p-5 border-b border-border">
            <div className="relative">
              ...
              <input placeholder="Search Layouts..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="ps-9 w-50 h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus:outline-none focus:ring-1 focus:ring-ring pl-9" />
          ...
          <div className="overflow-x-auto">
            {isLoading ? (
              ...
              <table className="w-full">
                <thead><tr className="border-b border-border">
                  ...
                  <th className="text-left px-5 py-3 text-muted-foreground font-medium text-sm">Region</th>
                  <th className="text-left px-5 py-3 text-muted-foreground font-medium text-sm">Action</th>
                </tr></thead>
                <tbody>
                  {paginatedData.map((layout: any) => (
                    <tr key={layout.id} className="border-b border-border hover:bg-muted/50">
                      <td className="px-5 py-3 text-foreground text-sm">{layout.name}</td>
                      <td className="px-5 py-3 text-foreground text-sm">{layout.regions?.name || '-'}</td>
                      <td className="px-5 py-3">
                        <div className="flex gap-2">
                          <Link to={`/manage/layout/${layout.id}/edit`} className="btn btn-warning text-xs px-3 py-1.5">Edit</Link>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            {!isLoading && paginatedData.length === 0 && <div className="text-center py-8 text-muted-foreground text-sm">No Layout found</div>}
          </div>
