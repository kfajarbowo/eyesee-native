  ...
  return (
    <ManageLayout>
      <Toolbar>
        <ToolbarHeading />
        <ToolbarActions>
          {hasPermission(user, "region.create") && (
            <Link to="/manage/region/create" className="btn">Create</Link>
          )}
        </ToolbarActions>
      </Toolbar>
      <Container>
        <div className="bg-card rounded-xl border border-border">
          <div className="p-5 border-b border-border">
            <div className="relative">
              ...
              <input placeholder="Search Regions..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="ps-9 w-50 h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus:outline-none focus:ring-1 focus:ring-ring pl-9" />
          ...
          <div className="overflow-x-auto">
            {isLoading ? (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedData.map((region: any) => (
                    <tr key={region.id} className="border-b border-border hover:bg-muted/50">
                      <td className="px-5 py-3 text-foreground text-sm">{region.name}</td>
                      <td className="px-5 py-3">
                        <div className="flex gap-2">
                          {hasPermission(user, "region.update") && <Link to={`/manage/region/${region.id}/edit`} className="btn btn-warning text-xs px-3 py-1.5">Edit</Link>}
                          {hasPermission(user, "region.delete") && <button className="btn btn-danger text-xs px-3 py-1.5">Delete</button>}
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            {!isLoading && paginatedData.length === 0 && <div className="text-center py-8 text-muted-foreground text-sm">No Region found</div>}
          </div>
