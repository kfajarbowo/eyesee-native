...
export default function BodyWormListPage() {
  const { user } = useAuth();
  ...
  const deleteMutation = useDeleteBodyWorm({ id: deleteModal.deleteId || "" });

  const bodyWormOnlyData = useMemo(() => {
    const allData = data?.data || [];
    return allData.filter((item: any) => item.type === 3 || item.type === "body_worm");
  }, [data]);

  const filteredData = useMemo(() => {
    if (!searchQuery) return bodyWormOnlyData;
    return bodyWormOnlyData.filter((item: any) => item.name?.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [searchQuery, bodyWormOnlyData]);
  ...
  return (
    <ManageLayout>
      <Toolbar>
        <ToolbarHeading />
        <ToolbarActions>
          {hasPermission(user, "body_worm.create") && (
            <Link to="/manage/body-worm/create" className="btn">Create</Link>
          )}
        </ToolbarActions>
      </Toolbar>
      <Container>
          ...
          <div className="overflow-x-auto">
            {isLoading ? (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedData.map((bwc: any) => (
                    <tr key={bwc.id} className="border-b border-border hover:bg-muted/50">
                      <td className="px-5 py-3 text-foreground text-sm">{bwc.name}</td>
                      <td className="px-5 py-3">
                        <div className="flex gap-2">
                          {hasPermission(user, "body_worm.update") && <Link to={`/manage/body-worm/${bwc.id}/edit`} className="btn btn-warning text-xs px-3 py-1.5">Edit</Link>}
                          {hasPermission(user, "body_worm.delete") && <button onClick={() => handleDelete(bwc.id)} className="btn btn-danger text-xs px-3 py-1.5">Delete</button>}
                          <Link to={`/manage/recording/body-worm/${bwc.path_slug}`} className="btn btn-secondary text-xs px-3 py-1.5">Recording Logs</Link>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            {!isLoading && paginatedData.length === 0 && <div className="text-center py-8 text-muted-foreground text-sm">No Body Worm found</div>}
          </div>
