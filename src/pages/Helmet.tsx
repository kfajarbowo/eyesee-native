...
export default function Helmet() {
  ...
  useEffect(() => {
    if (!isLoading && data?.data) {
      ...
      // Filter only Helmet cameras (type === 2)
      ...
      if (layoutArray.length > 0) {
        ...
        generatedLayout = layoutArray
          .filter((item) => helmetCameras.some((cctv: Cctv) => cctv.path_slug === item.i))
          .map((item) => {
            const matchedCctv = helmetCameras.find((cctv: Cctv) => cctv.path_slug === item.i);
            item.data = matchedCctv!;
            return item;
          });
      } else {
    
        generatedLayout = helmetCameras.map((cctv: Cctv, index: number) => ({
          i: cctv.path_slug,
    ...
    <DashboardLayout>
      ...
      {isLoading || isLoadingUserLayout || isLoadingLayout ? (
        ...
        <div className="flex flex-col items-center justify-center h-96">
          <p className="text-white text-xl">No Helmet cameras found</p>
          <p className="text-gray-400 text-sm mt-2">Please add Helmet cameras in the management page</p>
        </div>
      ) : (
        <GridLayout
